...
<template>
  ...
  >
    <div class="mb-8 flex items-center justify-between gap-2 px-1">
      <span
        v-if="!isCollapsed"
        class="min-w-0 truncate font-display text-[1.6rem] italic leading-none text-white"
      >
        {{ t('nav.brand') }}
      </span>
      <IconButton
        class="shrink-0 !text-ink-dark-text-soft hover:!bg-white/5 hover:!text-white"
    ...
    <nav class="flex flex-1 flex-col gap-1" :aria-label="t('nav.primary')">
      <RouterLink
        ...
        :key="item.to"
        :to="item.to"
        class="min-w-0 rounded-standard px-3 py-2 text-sm font-medium transition-colors"
        :class="[
